...
// створюємо функцію для підрахунку сумарного виграшу
// ##1 ** Designation of values ** Позначення величин ** Обозначение значений **
function getRandomNumber(minNumber, maxNumber) {
  return Math.floor(Math.random() * (maxNumber - minNumber + 1)) + minNumber
}
...
function getUserAmountOfWin(arr) {
  let userAnswer
  let sumWin = 0
  do {
...
const lotteryGame = getUserAmountOfWin(luckyNumbers)
// ##4 ** Display the result ** Виведення результату ** Вывод результата **
alert(`Результат вашого азарту ${lotteryGame}`)
