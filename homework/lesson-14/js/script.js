...
const summaryPrice = dataList.reduce((prevSum, { price }) => (price > 0 ? prevSum + price : prevSum), 0)
console.log(summaryPrice)

//Знайти кількість товарів, у яких ціна зменшилась (price < old_price).
const quantityProductsWithDiscount = dataList.reduce((prevQuantity, { price, old_price }) => (price < old_price ? prevQuantity + 1 : prevQuantity), 0)
console.log(quantityProductsWithDiscount)

//Товари, які доступні (sell_status:"available")
const availableProducts = dataList.filter(({ sell_status }) => sell_status === "available")
